...
type WorkspaceProps = { context: { industry: string; segment: string; audience: string; location: string; model: string; hours: string }; valueProp: ValueProp; setValueProp: (value: ValueProp) => void; onOpenStage: (stageId: number) => void };

function WorkspaceView({ context, valueProp, setValueProp, onOpenStage }: WorkspaceProps) {
  const [activeArtifact, setActiveArtifact] = useState('Problem brief');
  const artifacts = [
    { name: 'Founder thesis', stage: 'Stage 1', status: 'Ready', description: 'Why you are the person to build this.' },
    { name: 'Problem brief', stage: 'Stage 3', status: 'Draft', description: 'The problem, trigger, workaround, and cost.' },
    { name: 'Value proposition', stage: 'Stage 4', status: valueProp.outcome ? 'Ready' : 'Draft', description: 'The promise made to the first audience.' },
    { name: 'MVP experiment', stage: 'Stage 5', status: 'Next', description: 'The fastest credible test of the riskiest assumption.' },
    { name: 'First-customer plan', stage: 'Stage 6', status: 'Draft', description: 'The path to the first 10 customers or design partners.' },
    { name: 'Business model', stage: 'Stage 7', status: 'Draft', description: 'Who pays, for what, and at what economics.' }
  ];
  const selected = artifacts.find((artifact) => artifact.name === activeArtifact) ?? artifacts[0];
  return <div className="workstation-view"><div className="workspace-header"><div><p className="eyebrow">Founder workspace</p><h3>Your startup’s working memory.</h3><p>Keep the decisions, assumptions, and drafts that move the company forward in one place.</p></div><button onClick={() => onOpenStage(4)} className="primary-button">Create an artifact <PlusIcon className="h-4 w-4" /></button></div><div className="workspace-layout"><div className="workspace-artifacts">{artifacts.map((artifact) => <button key={artifact.name} onClick={() => setActiveArtifact(artifact.name)} className={`workspace-artifact-row ${activeArtifact === artifact.name ? 'workspace-artifact-active' : ''}`}><span className="workspace-artifact-icon"><DocumentTextIcon className="h-4 w-4" /></span><span><strong>{artifact.name}</strong><small>{artifact.stage} · {artifact.description}</small></span><em className={`artifact-status ${artifact.status.toLowerCase()}`}>{artifact.status}</em></button>)}</div><section className="workspace-editor"><div className="workspace-editor-top"><div><p className="eyebrow">{selected.stage} · {selected.status}</p><h4>{selected.name}</h4></div><button className="dashboard-link">Share draft</button></div>{selected.name === 'Value proposition' ? <div className="workspace-form"><label className="field-label">Customer promise<textarea value={valueProp.outcome} onChange={(event) => setValueProp({ ...valueProp, outcome: event.target.value })} placeholder={`What outcome will ${context.audience.toLowerCase()} get?`} /></label><label className="field-label">Core offering<textarea value={valueProp.offering} onChange={(event) => setValueProp({ ...valueProp, offering: event.target.value })} placeholder="Describe the smallest valuable offering..." /></label><div className="workspace-callout"><SparklesIcon className="h-4 w-4" /><span>Built for {context.audience} in {context.segment} · {context.industry}</span></div></div> : <div className="workspace-blank"><DocumentTextIcon className="h-7 w-7" /><h5>Start the {selected.name.toLowerCase()}</h5><p>{selected.description} FounderOps will turn this draft into a guided step when you are ready.</p><button onClick={() => onOpenStage(Number(selected.stage.replace('Stage ', '')))} className="secondary-button">Open guided step <ArrowRightIcon className="h-4 w-4" /></button></div>}</section></div><section className="operations-strip"><div><p className="eyebrow">Operating notes</p><h4>Make the work visible.</h4><p>Every artifact should connect to a decision, a customer signal, or a next action.</p></div><div className="operations-stats"><div><strong>1</strong><span>active assumption</span></div><div><strong>5</strong><span>customer conversations target</span></div><div><strong>Friday</strong><span>next review</span></div></div></section></div>;
}

function EvidenceView() {
  const [activeTab, setActiveTab] = useState('Signals');
  const [note, setNote] = useState('');
  const [signals, setSignals] = useState([{ title: 'Founder insight', type: 'Internal', detail: 'Preventable no-shows create lost revenue for independent clinics.', status: 'Confirmed' }]);
  const [experiment, setExperiment] = useState({ assumption: 'Clinic owners will change their workflow to reduce no-shows.', test: 'Interview 5 clinic owners and observe their reminder workflow.', success: '3 of 5 describe the problem as urgent and agree to test a solution.' });
  const addSignal = () => { if (!note.trim()) return; setSignals([...signals, { title: 'New customer signal', type: 'Interview note', detail: note, status: 'Unreviewed' }]); setNote(''); };
  return <div className="evidence-view"><div className="workspace-header"><div><p className="eyebrow">Evidence room</p><h3>Replace guesses with signals.</h3><p>Capture what customers say, what they do, and what your experiments teach you.</p></div><span className="evidence-counter">{signals.length} signals logged</span></div><div className="evidence-tabs">{['Signals', 'Experiments', 'Decisions'].map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={activeTab === tab ? 'evidence-tab-active' : ''}>{tab}</button>)}</div>{activeTab === 'Signals' && <div className="evidence-layout"><section className="evidence-card"><div className="evidence-card-heading"><div><p className="eyebrow">Log a signal</p><h4>What did you learn?</h4></div><UserGroupIcon className="h-5 w-5 text-slate-500" /></div><textarea value={note} onChange={(event) => setNote(event.target.value)} className="evidence-textarea" placeholder="Capture an exact quote, observed behavior, objection, or surprising pattern..." /><div className="evidence-card-footer"><span className="text-xs text-slate-500">Signals become evidence for a decision.</span><button onClick={addSignal} className="primary-button">Log signal <PlusIcon className="h-4 w-4" /></button></div></section><section className="evidence-card"><div className="evidence-card-heading"><div><p className="eyebrow">Customer evidence</p><h4>Signals so far</h4></div><span className="text-xs text-slate-500">Most recent</span></div><div className="signal-list">{signals.map((signal, index) => <div key={`${signal.title}-${index}`} className="signal-row"><span className="signal-mark">{signal.type === 'Internal' ? 'i' : '↗'}</span><div><strong>{signal.title}</strong><small>{signal.detail}</small><em>{signal.status}</em></div></div>)}</div></section></div>}{activeTab === 'Experiments' && <section className="evidence-card experiment-editor"><div className="evidence-card-heading"><div><p className="eyebrow">Active experiment</p><h4>Test the riskiest assumption first.</h4></div><span className="dashboard-status">Draft</span></div><label className="field-label">Assumption<textarea value={experiment.assumption} onChange={(event) => setExperiment({ ...experiment, assumption: event.target.value })} /></label><label className="field-label">Test method<textarea value={experiment.test} onChange={(event) => setExperiment({ ...experiment, test: event.target.value })} /></label><label className="field-label">Success signal<textarea value={experiment.success} onChange={(event) => setExperiment({ ...experiment, success: event.target.value })} /></label><button className="primary-button">Save experiment <CheckCircleIcon className="h-4 w-4" /></button></section>}{activeTab === 'Decisions' && <section className="evidence-card decision-list"><div className="evidence-card-heading"><div><p className="eyebrow">Decision log</p><h4>What changed because of evidence?</h4></div><button className="secondary-button">New decision</button></div><div className="decision-row"><span>01</span><div><strong>Focus on independent clinics first</strong><small>Selected as the beachhead because the founder has direct access and domain insight.</small></div><em>Persevere</em></div><div className="decision-row"><span>02</span><div><strong>Keep the first offering manual if needed</strong><small>Do not build a full platform until the no-show workflow is observed and validated.</small></div><em>Open</em></div></section>}</div>;
}
      ...
      <div className="mx-auto flex min-h-screen max-w-[1500px]">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8">
            ...
            {activeNav === 'Workspace' && <WorkspaceView context={context} valueProp={valueProp} setValueProp={setValueProp} onOpenStage={(stageId) => { setActiveStageId(stageId); setActiveNav('Build path'); }} />}
            {activeNav === 'Evidence' && <EvidenceView />}

            <div className={`${activeNav === 'Today' || activeNav === 'Workspace' || activeNav === 'Evidence' ? 'hidden' : 'grid'} gap-5 xl:grid-cols-[1fr_320px]`}>
